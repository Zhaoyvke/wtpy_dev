...
class LegData(HftContext):
    ...
    def update_comminfo(self,comminfo:ProductInfo)->None:
        #可以加个 isInit(false),在价差模块启动时候读入ProductInfo 
        
        self.min_volume = comminfo.minlots               #最小交易数量
        self.pricetick = comminfo.pricetick              #价格变动单位                     
        self.size = comminfo.volscale                    #数量乘数
        #待补充净头寸 net_position
    ...
    def update_position(self,stdCode)->None:
        ...
        if  self.net_pos >0: #多头
            self.long_pos = self.net_pos
            #elif position.direction == Direction.SHORT:
        elif self.net_pos <0: #空头
            self.short_pos = self.net_pos
