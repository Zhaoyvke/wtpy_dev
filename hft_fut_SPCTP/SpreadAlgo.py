...
from SpreadConstant import Direction, Status, Offset,OrderData,PositionData
...
class SpreadAlgo(SpreadAlgoTemplate,HftContext,BaseHftStrategy):
    ...
    def do_calc(self,newTick:dict)->None:
        ...
        if not self.is_order_finished():  #根据订单号判断完成情况
            return
...
#""""""""""""""""Template""""""""""""""
    ...
    def insert_tick_data(
        ...
        """"""
        self.leg_last_update = {}  # 存储每个腿最后更新的时间
        #更新数据于每个腿中
        ...
        """
        # 从价差类中读leg
        leg: LegData = self.spread.legs[newTick["stdCode"]]
        #leg: LegData = self.legs.get(newTick["stdCode"], None)
        if not leg:
            leg = self.get_leg(newTick["stdCode"])
        leg.update_tick(newTick)

        # 更新对应腿的tick数据
        self.leg_last_update[newTick["stdCode"]] = datetime.now()

        print("on_tick", newTick["stdCode"], "price:", newTick["price"], "A1price:", newTick["ask_price_0"], "A1qty:", newTick["ask_qty_0"], "B1price:", newTick["bid_price_0"], "B1qty:", newTick["bid_qty_0"], "tick:", newTick["action_time"])

        # 检查所有腿的更新时间
        now = datetime.now()
        for stdCode, last_update in self.leg_last_update.items():
            if now - last_update > 40:   #间隔时间>40s
                # 如果有腿的更新时间超过30秒，清空所有腿的数据
                for leg in spread.legs.values():
                    leg.clear_price()
                self.leg_last_update.clear()
                return

        # 如果所有腿都已更新，并且更新时间都在40秒内，调用do_calc函数
        if set(spread.legs.keys()) == set(self.leg_last_update.keys()):
            self.do_calc(newTick)

    def get_leg(self, stdCode: str) -> LegData:
        ...
        if not leg:
            ...
            self.get_contract
            comminfo= self.stra_get_comminfo(stdCode)
            if comminfo:
                leg.update_comminfo(comminfo)

            # Initialize leg position
            #positions:List[PositionData] = self.get_all_positions()
            #for position in positions:
            #    if position.stdcode == stdCode:
            #        leg.update_position(position)
            leg.update_position(stdCode)
