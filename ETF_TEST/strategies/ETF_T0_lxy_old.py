...
def get_snapshot_sz(df):
    ...
    df_min = df_min.with_columns(np.log(pl.col('vwap')).diff().alias('log_return'))
...
class ETF_T0_lxy(BaseCtaStrategy):
    ...
    def load_models(self):
        # 需要load models
        self.__rolling_model__ = TabularPredictor.load(f'{self.__path__}model/ag-20231123_093414/',require_py_version_match=False)
    ...
    def on_calculate(self, context:CtaContext):
        ...
        if min1_before == 930:
            part = new_data.filter((pl.col('OrigTime')<(date*1000000000+min1_before*100000)))
        else:
        ...
        # 如果信号连续 选择更新最新的时间
        print('判断信号连续 时间更新')
        print(f'历史信号-1 {df_all["position"].values[-1]}')
        if len(df_all) != 1:
            print(f'历史信号-2 {df_all["position"].values[-2]}')
        if len(df_all) == 1:
            if  (position!=0):
                self.position[self.current_time] = self.position[list(self.position.keys())[-1]]
                self.position.pop(list(self.position.keys())[-2])
        else:
            if (df_all['position'].values[-1]==df_all['position'].values[-2]) & (position!=0):
                self.position[self.current_time] = self.position[list(self.position.keys())[-1]]
                self.position.pop(list(self.position.keys())[-2])
