...
def timeToMaturity(today, month, trading_dates):
    ...
    elif len(str(month)) == 8:
        date = int(month)

    date_int = int(date)
    trading_dates_int = np.array([int(x) for x in trading_dates])
    if date_int not in trading_dates_int:
...
def etf_margin(s0, k, c, multiplier, optionType='c'):
    if optionType in ['c', 'C', 'Call', 'call']:  # call
        return (c + max(0.12 * s0 - max(k - s0, 0), 0.07 * s0)) * multiplier
    elif optionType in ['p', 'P', 'Put', 'put']:
        return min(c + max(0.12 * s0 - max(s0 - k, 0), 0.07 * k), k) * multiplier
...
def commodity_margin(s, c, multiplier, und_margin, k, optionType='c'):
    if optionType in ['c', 'C', 'Call', 'call']:  # call
        return c * multiplier + max(und_margin * 0.5, und_margin - 0.5 * max(k - s, 0))
    elif optionType in ['p', 'P', 'Put', 'put']:
        return c * multiplier + max(und_margin * 0.5, und_margin - 0.5 * max(s - k, 0))