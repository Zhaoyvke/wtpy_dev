...
class ETF_T0_lxy(BaseCtaStrategy):
    ...
    def on_session_begin(self, context: CtaContext, curTDate: int):
        if os.path.exists(f'F:/deploy/ETF_T0/daily_tick/{context.stra_get_date()}/{self.__code__}_min1.csv'):
            self.df=pd.read_csv(f'F:/deploy/ETF_T0/daily_tick/{context.stra_get_date()}/{self.__code__}_min1.csv')
        else:
    ...
    def on_init(self, context:CtaContext):
        ...
        if os.path.exists(f'F:/deploy/ETF_T0/daily_tick/{context.stra_get_date()}/{self.__code__}.csv'):
            tick_csv = pl.read_csv(f'F:/deploy/ETF_T0/daily_tick/{context.stra_get_date()}/{self.__code__}.csv')
            if len(tick_csv)>0:
                self.tick = tick_csv
            else:
    ...
    def on_calculate(self, context:CtaContext):
        ...
        new_data = self.tick
        # new_data = pd.read_csv(f'F:/deploy/ETF_T0/daily_tick/{self.__code__}.csv')  # 获取存储的tick数据
        columns_mapping ={
            'time': 'OrigTime',
            'code': 'ticker',
            'price': 'LastPx',
        ...
        # print(df_min.iloc[-1,:].values)
        self.df.loc[minute_to_index()[self.current_time],:] = df_min.iloc[-1,:].values
        self.df.to_csv(f'F:/deploy/ETF_T0/daily_tick/{context.stra_get_date()}/{self.__code__}_min1.csv',index=False)
        # print(data.columns)
        df = self.df
        ...
        df_all['pred_volume_ewma5'] = df_all.pred_volume.ewm(halflife=5).mean()
        df_all['pred_volume_ewma10'] = df_all.pred_volume.ewm(halflife=10).mean()
        df_all['pred_volume_ewma20'] = df_all.pred_volume.ewm(halflife=20).mean()
        df_all['pred_volume_stda'] = df_all.pred_volume.rolling(231*3,min_periods=1).std()
        print(f'time{df_all["minute"].values[-1]} volume{df_all["pred_volume"].values[-1]} volume_ma{df_all["pred_volume_ma"].values[-1]} volume_stda{df_all["pred_volume_stda"].values[-1]}')
        # df_all['position'] = np.where((df_all.pred_volume>(df_all.pred_volume_ma+1*df_all.pred_volume_stda)),1,0)
        df_all['position'] = np.where((df_all.pred_volume>(df_all.pred_volume_ma+0.5*df_all.pred_volume_stda)),1,0)*np.sign(df_all.OFI1)
        df_all['signal'] = df_all['position'].diff()

        df_all.to_csv(f'F:/deploy/ETF_T0/daily_tick/{context.stra_get_date()}/{self.__code__}_pred_vol.csv',index=False)
        ...
        # 需要重写
        context.set_position(code, self.__trdUnit__*position+self.__trdUnit__)
    ...
    def on_tick(self, context:CtaContext, stdCode:str, newTick:dict):
        ...
        if current_tick_time > last_tick_time:
            ...
            self.tick.write_csv(f'F:/deploy/ETF_T0/daily_tick/{context.stra_get_date()}/{self.__code__}.csv')
        # print(self.tick)
